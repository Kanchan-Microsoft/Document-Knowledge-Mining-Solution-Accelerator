...
public class AzureAISearchMemory : IMemoryDb, IMemoryDbUpsertBatch
{
    ...
    public async Task DeleteAsync(string index, MemoryRecord record, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            this._log.LogDebug("Deleting record {0} from index {1}", 
        ...
        {
            this._log.LogTrace("Index {0} record {1} not found, nothing to delete", 
                index.Replace("\r", string.Empty).Replace("\n", string.Empty), 
                id.Replace("\r", string.Empty).Replace("\n", string.Empty));
        }
    }
    ...
    private SearchClient GetSearchClient(string index)
    {
        ...
        this._log.LogTrace("Preparing search client, index name '{0}' normalized to '{1}'", 
            index.Replace("\r", string.Empty).Replace("\n", string.Empty), 
            normalIndexName.Replace("\r", string.Empty).Replace("\n", string.Empty));
